...
public class Number {
	...
	if ((digitssTemp = this.digits.get(Integer.valueOf(baseParam))) == null) {
	    ...
	    System.out.println(DateUtils.format(new Date()) + " digits " + this.base + ": "
		    + digitsInBase.substring(0, Math.min(digitsInBase.length(), 100)) + "... total: " + digitsInBase.length());
	...
	for (int i = 0; i < digitsInBase.length(); i++) {
	    if (digitsInBase.charAt(i) == ZERO) {
		current = i;
	    } else {
		if (digitsInBase.charAt(i) != ONE) {
		    return current;
		}
	    }
	}
